...
// Defines Express middleware that will upload our image
const uploadImage = async (req, res, next) => {

  // Define the absolute final file path for this image
  let uploadPath = `${__dirname}/../public/images/%s%s`

  // If the previous middleware did not pass un an ImageId then bail
  if(!req.imageId) { return }

  // Check to see if there are any files to upload
  if (Object.keys(req.files).length > 0) {
    // Get extension from the incoming file (ie: .png, .jpg, .gif)
    const extension = path.extname(req.files.image.name)

    // Render the final file path based off the req.imageId and file extension
